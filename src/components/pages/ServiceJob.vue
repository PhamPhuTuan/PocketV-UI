<template>
  <div class="container" id="top-page">
    <h2 style="color: #7cc141">Today's Jobs</h2>
    ...
    <!-- Upcoming Jobs -->
    <h2 style="color: #7cc141">Upcoming Jobs</h2>
    <h3 style="font-weight: bold">Search:</h3>
    <input
      class="form-control border input-search"
      id="myInput"
    ...
    <table class="table table-bordered table-striped table-custom">
      ...
      <tbody id="myTable">
        <tr v-for="item in upcomingJobListFiltered" :key="item">
          <td>{{ item.JobNum }}</td>
          <td>{{ item.Location }}</td>
          <td>{{ item.APPTTime }}</td>
          <td>{{ item.JobDescription }}</td>
    ...
    <p class="mt-5 mb-20">
      Showing 1 to {{ upcomingJobListFiltered.length }} of
      {{ upcomingJobListFiltered.length }} entries.
    </p>
    <!-- Recently Completed Jobs (last 90 days)-->
    <h2 style="color: #7cc141">Recently Completed Jobs (last 90 days)</h2>
    <h3 style="font-weight: bold">Search:</h3>
    <input
      class="form-control border input-search"
      id="myInput"
    ...
    <table class="table table-bordered table-striped table-custom">
      ...
      <tbody id="myTable">
        <tr v-for="item in recentlyCompletedJobList" :key="item">
          <td>{{ item.JobNum }}</td>
          <td>{{ item.Location }}</td>
          <td>{{ item.APPTTime }}</td>
          <td>{{ item.JobDescription }}</td>
    ...
    <p class="mt-5 mb-20">
      Showing 1 to {{ recentlyCompletedJobList.length }} of
      {{ recentlyCompletedJobList.length }} entries.
    </p>
  </div>
</template>
